...
app.post('/login', async (req, res) => {
    ...
    if (utilisateursConnectes.includes(username)) {
        return res.render('login', { error: 'Cet utilisateur est déjà connecté sur un autre appareil.' });
    }
    ...
    try {
      ...
      utilisateursConnectes.push(username); // Ajoute l'utilisateur à la liste des connectés
...
// function tirerCarte() {
//     const index = Math.floor(Math.random() * cartes.length);
//     const carteTiree = cartes[index];
//     cartes.splice(index, 1); // Retire la carte du tableau
//     return carteTiree;
// }
...
function prochaineManche() {
    ...
    io.emit('messageFinGme', messageFinGame); // Envoi le message ici
    }
}
...
app.get('/logout', (req, res) => {
    ...
    // listeUsers = listeUsers.filter(user => user !== username);
    utilisateursConnectes = utilisateursConnectes.filter(user => user !== username); // Supprime l'utilisateur de la liste des connectés
    utilisateursConnectes = utilisateursConnectes.filter(user => user !== username); // Supprime l'utilisateur de la liste des 
