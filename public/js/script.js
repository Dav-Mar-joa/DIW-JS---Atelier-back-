...
document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on('miseAJourTemps', (tempsEcoule) => {
        elapsedTime = tempsEcoule; // Met à jour le temps écoulé
    ...
    envoyerMessageButton.addEventListener('click', () => {
        ...
        const joueurId = joueurs[0] ? 1 : 2; // Identifie le joueur actuel
        if (message) {
            socket.emit('messageChat', message, joueurId); // Envoi le message avec l'ID du joueur
            messageInput.value = '';
        }
    });
...
socket.on('nouveauMessage', (message) => {
    ...
    messageElement.textContent = message; // Affiche le message avec l'auteur
    messagesContainer.appendChild(messageElement);
    messagesContainer.scrollTop = messagesContainer.scrollHeight;  // Fait défiler vers le bas pour voir le dernier message
});
